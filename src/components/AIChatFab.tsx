...
export default function AIChatFab() {
  ...
  const handleQuickAction = (action: string) => {
    ...
    setInput(locale === 'es' ? prompts[action].es : prompts[action].en);
  };
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-pink-500 to-rose-500 shadow-xl shadow-pink-300/50 transition-all duration-300 hover:scale-110 animate-pulse-glow"
        >
      ...
      {/* Chat panel */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 flex h-[520px] w-[380px] flex-col overflow-hidden rounded-3xl shadow-2xl shadow-pink-200/40 animate-fade-in-up border border-white/50">
          {/* Header */}
          <div className="flex items-center justify-between bg-gradient-to-r from-pink-500 via-rose-400 to-rose-500 px-4 py-3.5 shrink-0">
          ...
          {/* Messages */}
          <div className="flex-1 space-y-3 overflow-y-auto px-4 py-3 bg-gradient-to-b from-pink-50/50 to-white/80 backdrop-blur-sm">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'assistant' && (
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-pink-400 to-rose-500 shadow-sm mr-2 mt-0.5">
                    <Bot className="h-3.5 w-3.5 text-white" />
                  </div>
                )}
                <div className={`max-w-[80%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed shadow-sm ${
                  msg.role === 'user'
                    ? 'bg-gradient-to-br from-pink-500 to-rose-500 text-white rounded-br-sm animate-slide-in-right'
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-pink-400 to-rose-500 shadow-sm mr-2">
                  <Bot className="h-3.5 w-3.5 text-white" />
                </div>
                <div className="rounded-2xl rounded-bl-sm bg-white/90 border border-white/60 px-4 py-3 backdrop-blur-sm">
                  <div className="flex gap-1.5 items-center">
                    ...
                    <span className="h-2 w-2 rounded-full bg-pink-400 animate-bounce" style={{ animationDelay: '150ms' }} />
                    <span className="h-2 w-2 rounded-full bg-pink-400 animate-bounce" style={{ animationDelay: '300ms' }} />
                  </div>
                </div>
              </div>
            )}
          ...
          {/* Quick actions */}
          <div className="flex gap-1.5 px-4 py-2 border-t border-pink-100/40 bg-white/70 backdrop-blur-sm">
            {[
          ...
          {/* Input */}
          <div className="flex items-center gap-2 px-4 py-3 border-t border-pink-100/40 bg-white/80 backdrop-blur-sm shrink-0">
            <input
              ref={inputRef}
              value={input}
              ...
              className="flex-1 rounded-xl border border-pink-100/60 bg-white/70 px-3.5 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-pink-300 backdrop-blur-sm transition-base"
            />
            <button
              onClick={sendMessage}
              disabled={!input.trim()}
              className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-pink-500 to-rose-500 text-white shadow-md shadow-pink-200/60 transition-all hover:opacity-90 hover:scale-105 disabled:opacity-40 disabled:cursor-not-allowed"
            >
